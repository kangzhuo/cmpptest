...
class CmppPackDataLT {
    ...
    byte[] makeCmppConnectReq() throws Exception {
        ...
        byte[] sourceAddr = CmppPackDataLT.config.sourceAddr.getBytes();
        byte[] authenticatorSource = CmppUtil.MD5(CmppPackDataLT.config.sourceAddr + new String(l_bytes) + CmppPackDataLT.config.pwd + l_strTime);
        byte[] version = new byte[]{CmppPackDataLT.config.version};
        byte[] timestamp = CmppUtil.int2byte(Integer.parseInt(l_strTime));

        byte[] head = makeHead(CmppPackDataLT.CMPP_CONNECT, 27, new byte[] {(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x01});
    ...
    byte[] makeCmppSubmitReq(int p_iSeq, String p_strTel, int p_iMsgType, String p_strMsg) throws IOException {
        ...
        registeredDelivery = new byte[] {CmppPackDataLT.config.registeredDelivery};    //是否要求返回状态确认报告：0：不需要 1：需要 2：产生SMC话
        msgLevel = new byte[] {CmppPackDataLT.config.msgLevel};    //信息级别
        serviceId = CmppUtil.str2ByteHead("xsms", 10);
        feeUserType = new byte[] {CmppPackDataLT.config.feeUserType};
        ...
        msgSrc = CmppPackDataLT.config.spId.getBytes();
        feeType = CmppPackDataLT.config.feeType.getBytes();
        feeCode = CmppUtil.str2ByteHead(CmppPackDataLT.config.feeCode, 6);
        validTime = CmppPackDataLT.config.validTime;
        atTime = CmppPackDataLT.config.atTime;
        srcId = CmppUtil.str2ByteHead(CmppPackDataLT.config.srcId, 21);
        destUsrTl = new byte[] {(byte)0x01};
        destTerminalId = CmppUtil.str2ByteHead(p_strTel, 21);
        reserve = CmppPackDataLT.config.reserve;

        head = makeHead(CmppPackDataLT.CMPP_SUBMIT, 147 + l_iMsgLength, CmppUtil.int2byte(p_iSeq));
    ...
    byte[] makeCmppDeliverReq(int p_iSeq, byte[] msgId, byte result) throws IOException {
        ...
        byte[] head = makeHead(CmppPackDataLT.CMPP_DELIVER | 0x80000000, 9, CmppUtil.int2byte(p_iSeq));
    ...
    byte[] makeCmppQueryReq() throws IOException {
        ...
        byte[] head = makeHead(CmppPackDataLT.CMPP_QUERY, 27, new byte[] {(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x02});
    ...
    byte[] makeCmppActiveTestReq() throws IOException {
        byte[] head = makeHead(CmppPackDataLT.CMPP_ACTIVE_TEST, 0, new byte[] {(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x03});
    ...
    byte[] makeCmppActiveTestResp(int p_iSeq) throws IOException {
        byte[] head = makeHead(CmppPackDataLT.CMPP_ACTIVE_TEST | 0x80000000, 1, CmppUtil.int2byte(p_iSeq));
    ...
    Map<String,Object> readCmppConnectResp(byte[] p_respDatas) {
        ...
        if (CmppPackDataLT.CMPP_CONNECT != (l_iCmppType & 0x000000ff)) { //仅需要命令的最后一字节
            return null;
        }
    ...
    private byte[] makeMMSBody(String p_strTel, String p_strMsg) throws IOException {
        ...
        byte[] x_mms_src = CmppUtil.str2Byte(CmppPackDataLT.config.srcId, CmppPackDataLT.config.srcId.length());
